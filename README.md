...
- **A resolver that fails** by throwing, rejecting or passing `themeForTimeoutMs` (1,000 unless set) leaves its theme out of the call, and the call goes on. The bot logs a server or user whose lookup fails once, and again when it answers, and does not ask it again for 10 seconds; when more than one fails at a time, it logs one line for the resolver instead, and one more when every one answers again.
