...
- `overrideTheme(theme)` on the builder replaces the app's `@MeoCord({ theme })` for that module, or gives a module without an app one, beneath each `@UseTheme`; `overrideThemeFor(resolvers)` replaces the app's `themeFor`, or removes it with `undefined`. Both are checked as `@MeoCord` checks them.
